...
@section('content')
    <div class="container mx-auto px-4 py-8">
        <h1 class="text-3xl font-bold text-gray-800 mb-6">Daftar Teknisi</h1>

        <a href="{{ route('admin.teknisi.create') }}"
            class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 mb-6">
                ...
                <thead>
                    <tr>
                        ...
                        <th
                            class="px-5 py-3 border-b-2 border-gray-200 bg-gray-100 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">
                            Aksi
                        </th> {{-- Kolom untuk tombol aksi (lihat, edit, hapus) --}}
                    </tr>
                </thead>
                <tbody>
                    @forelse ($teknisi as $teknisiItem)
                        {{-- Mengubah nama variabel untuk menghindari konflik dengan $teknisi di luar loop --}}
                        <tr>
                            <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                <p class="text-gray-900 whitespace-no-wrap">{{ $teknisiItem->id }}</p>
                            </td>
                            <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                <p class="text-gray-900 whitespace-no-wrap">{{ $teknisiItem->name }}</p>
                            </td>
                            <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                <p class="text-gray-900 whitespace-no-wrap">{{ $teknisiItem->email }}</p>
                            </td>
                            <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                <p class="text-gray-900 whitespace-no-wrap">{{ ucfirst($teknisiItem->role) }}</p>
                            </td>
                            <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                <p class="text-gray-900 whitespace-no-wrap">
                                    {{ $teknisiItem->created_at->format('d M Y H:i') }}</p>
                            </td>
                            <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                <div class="flex items-center space-x-2">
                                    ...
                                    <a href="{{ route('admin.teknisi.show', $teknisiItem->id) }}"
                                        ...
                                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
                                            ...
                                            stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-eye mr-1">
